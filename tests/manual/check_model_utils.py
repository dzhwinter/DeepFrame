...
left.add(MaxPoolinhg2D(poolsize=(2,2)))
left.add(Flatten())
left.add(Dense(32 * 13 * 13, 50))

right = Sequential()
right.add(Dense(784, 30))
right.add(Dense('relu'))
...
graph.add_node(Dense(32, 10), name='dense1', input='input1')
...
graph.add_node(name='output1', input=['dense1', 'dense2'], merge_mode='sum')
graph.add_output(name='output2', input=['dense3', 'dense4'],merge_mode='concat')

layer_utils.print_layer_shapes(graph, {'input1':(1, 32), 'input2': (1,1,28, 28)})
print('Tets scirpt complete')
